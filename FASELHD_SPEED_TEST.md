...
## 19) إصلاحات تجربة المستخدم (بعد ملاحظات التشغيل الأول)

1. **لا حذف بعد الآن**: عند إرسال الرابط يظهر مؤشر `typing/upload_photo` أثناء التحليل ثم تُرسل **رسالة صورة واحدة** بالأزرار — وكل المراحل بعدها **تعديل** لنفس الرسالة. الاستخراج التفاعلي سريع (3 محاولات × 12s بدل الانتظار الطويل).
2. **المراحل مضمونة الظهور + تشخيص**: مسار `/fail` يعيد أي مهمة متعثرة للطابور ويبلغ المستخدم (بدل التجمد الصامت)، وأمرا `/diag` و`/status` يعرضان الطابور والنشط ونبض العمال، وendpoint `/diag` يعطي نفس البيانات للمراقبة الخارجية، وheartbeat كل 60s من كل عامل (مُتحقق: w1..w4 تنبض).
3. **الخفة**: لا تغيير معماري — البطء المُلاحظ كان تسلسل (رسالة انتظار + حذف + إعادة إرسال) لا ثقل معالجة.
4. **الفيديو يُرفع بصورة الغلاف**: العامل يحمّل `thumbnail` المهمة ويمررها لـ `tg_upload.py --thumb` (تُضغط <200KB شرط تليجرام وتُرفق كـ `thumbnail` في `sendVideo`) — فيظهر الغلاف المستخرج من الموقع على كل فيديو.
 Workflow يقبل `runtime_minutes` (دورات debug قصيرة) و`auto_cycle` (إيقاف إعادة التشغيل الذاتية عند التشخيص).

## 20) تشخيص الدورات المكررة + إثبات الـ thumbnail

**السبب الجذري لرسالتين/غياب المراحل:** تبيّن من `gh run list` وجود **دورتي مزرعة متوازيتين** (34599208848 بجانب 34599321463) — مثيلا بوت رئيسي يتسابقان على نفس الـ token: تحديث يُسلَّم لأحدهما arbitrarily، والنقر قد يصل للمثيل الذي لا يملك المهمة (صمت تام)، والصور تتكرر. والأسوأ: إلغاء أي دورة كان يشغّل `cycle` (بشرط `always()`) فيولّد دورة بديلة — **حلقة إلغاء-يتوالد** رُصدت 4 دورات متعاقبة.
